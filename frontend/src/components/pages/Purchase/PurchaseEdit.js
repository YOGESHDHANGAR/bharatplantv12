...
import { useUpdatePurchase } from "../../../api/mutations/purchaseMutations";
...
let PurchaseEntryRenderCount = 0;

const PurchaseEdit = ({ selectedPurchase }) => {
  ...
  const {
    ...
    defaultValues: {
      _id: selectedPurchase._id || "",
      vchNo: selectedPurchase.vchNo || "",
      date: selectedPurchase.date
        ? moment(selectedPurchase.date).format("DD-MM-YYYY")
        : "",
      ...
      narration: selectedPurchase.narration || "",
      voucherTotal: selectedPurchase.voucherTotal || null,
      ledger: {
        value: selectedPurchase.ledger,
        label: selectedPurchase.partyName,
      },
    },
    mode: "onChange",
  });
  ...
  // Call UpdatePurchase hook
  const { mutateAsync: updatePurchaseAsync } = useUpdatePurchase();
  ...
  const onSubmit = async (formData) => {
    ...
    await updatePurchaseAsync(formData);
...
export default React.memo(PurchaseEdit);
