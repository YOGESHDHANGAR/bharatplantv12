...
const ReceiptEdit = ({ selectedReceipt }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      vchNo: selectedReceipt.vchNo || "",
      date: selectedReceipt.date
        ? moment(selectedReceipt.date).format("DD-MM-YYYY")
        : "",
      ref: selectedReceipt.ref || "",
      day: selectedReceipt.day || "",
      partyName: selectedReceipt.partyName || "",
      mode: {
        mode: null,
        modeType: "",
        modeAmount: null,
      },
      modes: selectedReceipt.modes || [],
      narration: selectedReceipt.narration || "",
      voucherTotal: selectedReceipt.voucherTotal || null,
      ledger: {
        value: selectedReceipt.ledger,
        label: selectedReceipt.partyName,
      },
    },
    mode: "onChange",
  });
  ...
  const {
    data: {
      getAllLedgersResult: fetchedLedger = [], // Access the receipts data array directly
      totalLedgers,
      currentPage,
      totalPages,
    } = {},
  ...
  // Call UpdateReceipt hook
  const { mutateAsync: updateReceiptAsync } = useUpdateReceipt();
  ...
  const onSubmit = async (formData) => {
    ...
    await updateReceiptAsync(formData);
...
export default React.memo(ReceiptEdit);
