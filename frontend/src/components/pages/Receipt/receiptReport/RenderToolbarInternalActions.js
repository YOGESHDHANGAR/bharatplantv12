...
const generatePDF = (data) => {
  ...
  doc.save("receipts-report.pdf");
};

// Generate Excel
const generateExcel = (data) => {
  if (!data || data.length === 0) {
    console.error("No data available to generate Excel file");
    return;
  }

  const ws = XLSX.utils.json_to_sheet(data); // Convert JSON data to sheet
  const wb = XLSX.utils.book_new(); // Create a new workbook
  XLSX.utils.book_append_sheet(wb, ws, "Receipts Report"); // Append the sheet
  XLSX.writeFile(wb, "receipts-report.xlsx"); // Write to file
};
  ...
  const handleDownload = async (format) => {
    ...
    switch (format) {
      ...
      case "CSV":
        ...
        link.setAttribute("download", "receipts-report.csv");
